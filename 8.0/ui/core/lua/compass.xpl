...
-- like. Their real names exist only in the menus environment, and
-- GetLiveDataBridge is the one channel into it from a core script. Answers are
-- cached per control id and dropped whenever the menus side flags a change.
local function refreshHotkeyNames()
	if C.GetConfigSetting(config.hotkeyNameDirtySetting) ~= 1 then
		return
	end
	private.hotkeyNames = {}
	private.hotkeyNamesDebug = C.GetConfigSetting(config.hotkeyNameDebugSetting) == 1
	-- Cleared before the refetch rather than after, so a registration landing
	-- mid-refresh re-raises it instead of being swallowed.
	C.SetConfigSetting(config.hotkeyNameDirtySetting, false)
end

local function getHotkeyName(controlid)
	local cached = private.hotkeyNames[controlid]
	if cached then
		return cached
	end
	local name = GetLiveDataBridge(config.hotkeyNamePlaceholder, tostring(C.GetPlayerID()), tostring(controlid))
	if name == nil then
		-- Menus side did not answer - don't cache, ask again on the next open.
		return ""
	end
	private.hotkeyNames[controlid] = name
	if private.hotkeyNamesDebug then
		DebugError(string.format("Hotkey Names on Radial Menu: control %d resolved to '%s'", controlid, name))
	end
	return name
end
